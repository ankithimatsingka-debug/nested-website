...
const MobileComparisonCard = ({ product, data }: { product: string; data: any }) => {
  ...
  return (
    ...
    >
      ...
      {!productData.verdict && (
        <p className="text-xs">{productData.text}</p>
      )}
    </div>
  );
};
        ...
        >
          ...
          <div className="space-y-1 text-xs">
            <div className="flex justify-between">
              ...
              <span className="font-medium">{item.safety}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Flexibility:</span>
              <span className="font-medium">{item.flexibility}</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">{item.taxEfficiency}</span>
            </div>
          </div>
        </Card>
      ))}
                ...
                <AccordionContent className="px-4 pb-4">
                  <div className="grid grid-cols-2 gap-3">
                    {investmentProducts.map((product) => (
                      <div key={product.key}>
                        <div className="text-xs font-semibold text-muted-foreground border-b pb-1 mb-2">
                          {product.shortName}
                        </div>
